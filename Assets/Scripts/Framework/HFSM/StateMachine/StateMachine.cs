...
namespace JLXB.Framework.FSM
{
		...
		IActionable<TEvent>
	{
		...
		{
			if (activeState == null)
				throw JLXB.Framework.FSM.Exceptions.Common.NotInitialized(context);
		}
		...
		private void ChangeState(TStateId name, ITransitionListener listener = null)
		{
			...
			{
				throw JLXB.Framework.FSM.Exceptions.Common.StateNotFound(name.ToString(), context: "Switching states");
			}
		...
		public override void OnEnter()
		{
			...
			{
				throw JLXB.Framework.FSM.Exceptions.Common.MissingStartState(context: "Running OnEnter of the state machine.");
			}
		...
		public StateBase<TStateId> GetState(TStateId name)
		{
			...
			if (!stateBundlesByName.TryGetValue(name, out bundle) || bundle.state == null)
			{
				throw JLXB.Framework.FSM.Exceptions.Common.StateNotFound(name.ToString(), context: "Getting a state");
			}
		...
		public StateMachine<string, string, string> this[TStateId name]
		{
			get
			{
				...
				if (subFsm == null)
				{
					throw JLXB.Framework.FSM.Exceptions.Common.QuickIndexerMisusedForGettingState(name.ToString());
				}
