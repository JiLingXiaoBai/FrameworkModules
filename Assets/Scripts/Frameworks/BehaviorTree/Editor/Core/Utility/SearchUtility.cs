...
namespace JLXB.Framework.BehaviorTree.Editor
{
    ...
    {
        public static IEnumerable<IGrouping<string, Type>> GroupsByEditorGroup(this IEnumerable<Type> types)
        {
            return types.GroupBy(t =>
            {
                var array = t.GetCustomAttributes(typeof(EditorGroupAttribute), false) as EditorGroupAttribute[];
        ...
        {
            return groups.SelectMany(x => x).Where(x => x.IsSubclassOf(Father)).GroupsByEditorGroup();
        }
